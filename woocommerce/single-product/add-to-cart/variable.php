...
// Hide this array of attribute from product page because we already choose them
$exception = [];
if (isset($_COOKIE['attributes'])) {
	$chosen_attributes_keys = json_decode(stripslashes($_COOKIE["attributes"]));
	foreach ($chosen_attributes_keys as $key => $val) {
		$exception['pa_' . $key] = $val;
	}
}
